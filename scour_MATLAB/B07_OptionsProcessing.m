...
elseif all(Beam.BC.text == 'UD')
    % Check for defined number of spans, default to 2 if not present
    if isfield(Beam.Prop, 'num_spans')
        num_spans = Beam.Prop.num_spans;
    else
        num_spans = 2;
    end

    % Automatically distribute supports 
    Beam.BC.loc = linspace(0, Beam.Prop.L, num_spans + 1);
    
    % Turn ON vertical stiffness for ALL supports (value = 1)
    Beam.BC.vert_stiff = ones(1, length(Beam.BC.loc));
    
    % Turn ON rotational stiffness ONLY for abutments (first and last)
    rot_stiff = zeros(1, length(Beam.BC.loc));
    rot_stiff(1) = 1;       % Entrance abutment
    rot_stiff(end) = 1;     % Exit abutment
    Beam.BC.rot_stiff = rot_stiff;
    
    Beam.BC.text = 'User-defined';
    Beam.BC.text_long = sprintf('%d-span bridge', num_spans);
