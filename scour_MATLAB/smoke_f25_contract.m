...
try
    ttbi.f25_bundle_source_binding('', 'invalid', '', 0, cfg_r);
    error('smoke_f25_contract:BundleBindingAcceptedInvalidInput', ...
        'F25 bundle-source binding accepted a malformed identity.');
catch bundle_error
    assert(strcmp(bundle_error.identifier, ...
        'ttbi:f25_bundle_source_binding:Inputs'));
end
...
provenance = struct( ...
    'campaign_matlab_environment_sha256', repmat('a', 1, 64), ...
    'actual_matlab_environment_sha256', repmat('b', 1, 64), ...
    'generator_source_root_sha256', repmat('c', 1, 64), ...
    'bundle_source_binding_sha256', repmat('d', 1, 64), ...
    'qualification_source_sha256', 'PRODUCTION');
identity = ttbi.f25_generation_identity(cfg_r, design_r, provenance);
mutated = provenance;
mutated.actual_matlab_environment_sha256 = repmat('e', 1, 64);
mutated_identity = ttbi.f25_generation_identity( ...
    cfg_r, design_r, mutated);
assert(~strcmp(identity.gen_fingerprint, ...
    mutated_identity.gen_fingerprint));
mutated = provenance;
mutated.bundle_source_binding_sha256 = repmat('f', 1, 64);
mutated_identity = ttbi.f25_generation_identity( ...
    cfg_r, design_r, mutated);
assert(~strcmp(identity.gen_fingerprint, ...
    mutated_identity.gen_fingerprint));
