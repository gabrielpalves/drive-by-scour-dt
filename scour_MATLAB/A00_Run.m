...
fp_cfg = struct( ...
    ...
    ... % CODE-BEHAVIOUR version (audit r3, 2026-07-22). The r3 fixes changed
    ... % RULES while leaving every knob VALUE untouched: hanging<->fouling
    ... % acceptance odds 9:1 -> 3:1, overlapping ballast patches product ->
    ... % governing-patch, B66 on-track mask bounded to [0, Profile.L]. Without
    ... % this key the fingerprint would be IDENTICAL across the change and the
    ... % resume guard would silently MIX pre- and post-fix states in one
    ... % dataset (the exact failure mode the R2 schema guard exists to stop).
    ... % Bump this string whenever generation BEHAVIOUR changes without a knob.
    'gen_rule_ver', 'r3-2026-07-22-hangodds3to1-patchgovern-b66bounded', ...
...
% Track implementation revision is conditional so unaffected existing R8 rungs
% retain their valid fingerprint. Track rungs being regenerated after the
% governing-patch fix cannot resume a pre-fix partial folder under the same R8
% schema/fingerprint.
track_eov_impl = 'governing-abs-log-patch-v1';
if use_track_eov
    fp_cfg.track_eov_impl = track_eov_impl;
