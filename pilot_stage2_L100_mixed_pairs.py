"""
pilot_stage2_L100_mixed_pairs.py
=================================
MIXED-PAIR PILOT on the EXISTING (L100, deprecated-geometry) Stage-2 data.

Purpose (2026-07-12; see docs/framework_rationale.md Stage-2 forensics entry):
the per-passage EOV randomization collapsed every sprung channel while the
wheel pair survived. The deep-research report (papers/'Drive-By Scour ML
Literature Design') supports TSD-residual-style FUSION: an unsprung channel
(profile reference) + a sprung channel (inertial bridge response) should beat
the wheel+wheel pair. This pilot tests exactly that on the data already on the
Lab PC - zero regeneration cost; its qualitative answer informs sensor
selection for every later stage, even though the L100 dataset itself is
superseded by the L=99.6 regeneration.

RUN: copy this file NEXT TO the existing comprehensive_ablation_multidamage.py
on the Lab PC (same folder that holds core/, training/, data/, database/,
results/) and run it. All previously completed studies are SKIPPED (pipeline
resume); only the two new mixed-pair studies train (75 trials each, seed 42 -
matched to the existing Stage-2 budget for comparability). The summary
(including the old singles + wheel pair + the new mixed pairs) is written to
results/stage2_4span_L100pilot_summary/ - it does NOT touch the original
stage2_4span_summary. Runs fine in parallel with a MATLAB generation job.

Based on comprehensive_ablation_multidamage.py (docstring below).
--------------------------------------------------------------------------
...
STAGE = "stage2_4span"   # PILOT: fixed - do not change
...
if STAGE == "stage0_multiscour":
    ...
    # PILOT OVERRIDE: the dataset that ACTUALLY exists on the Lab PC - the
    # deprecated pre-fix L=100 geometry with the old per-PASSAGE EOV draws
    # (crackON / prof-psd_fra classes {4,5,6}). Matches the folder the original
    # Stage-2 run trained on, so all its studies resume/skip untouched.
    DATASET         = "L100_4span_multi_scour_scourS2-3-4_bearTGT_crackON_prof-psd_fra_dano0-60pct_states271_Npass50_varNVST"
...
N_TRIALS       = 75           # PILOT: match the existing Stage-2 study budget
                              # (75 trials/seed42) so the mixed pairs are
                              # directly comparable AND the old studies skip.
EPOCHS         = 50
SEEDS          = [42]         # PILOT: single seed - comparability with the
                              # existing L100 leaderboard, minimal PC time.
...
# THE PILOT: two mixed unsprung+sprung fusion pairs (wheel = profile
# reference, sprung = inertial bridge response; TSD-residual logic).
# Wheel1_Vert (best single overall on L100) + the two best sprung singles:
# FrontBogie_Vert (best sprung) and CarBody_Vert (the literature's canonical
# fusion partner). DOF map: 0 CarBody_Vert, 1 FrontBogie_Vert,
# 2 RearBogie_Vert, 3 Wheel1_Vert, 4 Wheel2_Vert, 5 CarBody_Pitch,
# 6 FrontBogie_Pitch, 7 RearBogie_Pitch.
EXTRA_PAIRS: list[list[int]] = [[1, 3], [0, 3]]

# PILOT: separate summary dir - the original stage2_4span_summary (the L100
# forensic record) is left untouched.
SUMMARY_DIR = os.path.join("results", "stage2_4span_L100pilot_summary")
